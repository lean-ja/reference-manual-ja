...
型クラスのパラメータは {name}`outParam` {tech}[ガジェット] でそれらの型をラップすることで出力として宣言することができます。クラスのパラメータが出力である場合、インスタンス統合はそれが既知であることを要求しません；実際、どんな値を渡しても完全に無視されます。入力パラメータがマッチする最初のインスタンスが選択され、そのインスタンスの出力パラメータの値には利用箇所で使用される値が割り当てられます。そのインスタンスの出力パラメータに値が存在していた場合、統合が完了した後にその値と割り当てられた値が比較され、一致しない場合はエラーとなります。
...
半出力パラメータは統合を開始する前に知っている必要がないという点では出力パラメータと同じです；出力パラメータとは異なり、インスタンスを選択する際にその値が考慮されます。
