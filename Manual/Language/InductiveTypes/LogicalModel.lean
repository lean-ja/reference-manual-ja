...
論理に新しい定数が追加されるだけでなく、帰納的データ型宣言には新しい簡約規則も追加されます。これらの規則は再帰子とコンストラクタ、特にコンストラクタをターゲットとする再帰子の相互作用を支配します。この簡約の形式は {deftech}_ι簡約_ {index}[ι-reduction]{index (subterm:="ι (iota)")}[reduction] （iota reduction）と呼ばれます。
...
帰納的データ型の宣言は、多くの適格要件に従います。これらの要件は、Lean が帰納的データ型の新しい規則で拡張された時に、論理としての一貫性が保たれることを保証します。これらの要件は保守的です：一貫性を損なわない帰納型だったとしてもこれらの要件はその帰納型を拒否する可能性があります。
...
`Bad` というデータ型はもし不許可でなければ Lean を矛盾した存在にしてしまいます：
