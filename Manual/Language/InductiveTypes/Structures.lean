...
{deftech}[構造体] （structure）は単一のコンストラクタと添字を持たない非再帰的帰納型です。これらの制限と引き換えに、Lean は構造体のための数々の便利なコードを生成します：アクセサ関数が各フィールドに対して生成される・位置引数ではなくフィールド名に基づく追加のコンストラクタ構文が利用できる・同様の構文を使用して特定の名前付きフィールドの値を書き換えることができる・構造体は他の構造体を拡張することができる。構造体は Lean の表現力を増すものではありません；すべての機能はコード生成の観点から実装されます。
...
各フィールドに対してベースとなるデータ型のコンストラクタからフィールドの値を抽出する {deftech}[射影関数] （projection function）が生成されます。この関数は構造体の名前の名前空間に存在します。構造体のフィールドの射影はエラボレータによって特別に処理され（ {ref "structure-inheritance"}[構造体の継承についての節] で説明します）、名前空間を検索する以上の余分なステップが実行されます。フィールドの型が先行するフィールドに依存する場合、依存する射影関数の型は、明示的なパターンマッチではなく、先行する射影の観点から記述されます。
...
構造体は単一コンストラクタの帰納型として表現されるため、そのコンストラクタは {tech}[匿名コンストラクタ構文] を使用して呼び出したり、マッチしたりすることができます。さらに、構造体はフィールドの名前とフィールドの値を使って構築したり、マッチすることができます。
