...
:::
  ...
  : インスタンス暗黙のパラメータ
    ...
    インスタンス暗黙のパラメータの引数は {ref "instance-synth"}[型クラス合成] を介して見つけられます。インスタンス暗黙のパラメータは角括弧（`[` と `]`）で囲まれ、ほとんどの場合パラメータ名は省略されます。というのも、パラメータとして関数に渡されるインスタンス統合は名前を明示的にしなくともすでに関数本体で利用可能であるからです。
