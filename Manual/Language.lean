...
-/
#doc (Manual) "Lean 言語について（The Lean Language）" =>
...
命題を返す関数である述語（つまり、関数の結果の型が `Prop` にある型である場合）は引数の型がどのような宇宙に会っても構いませんが、関数の型自体は `Prop` に留まります。言い換えると、命題は {deftech}[_impredicative_] {index}[impredicative]{index subterm := "impredicative"}[quantification] な量化子を特徴づけます。というのも、命題はそれ自体、すべての命題（および他のすべての命題）についての文になりうるからです。
...
これは次の例で見ることができます。 `T` は不要な宇宙多相定義で、常にユニット型のコンストラクタを返します。`T` のインスタンスはどれも同じ値を持ち、また同じ型を持ちますが、宇宙のインスタンス化が異なるため互換性がありません：
...
:::
::::Manual.example "自動的に束縛された宇宙のモノ射"
:::comment
...
`autoImplicit` を使うことに加えて、`universe` コマンドを使って特定の識別子を特定の {tech}[scope] 内の宇宙変数として宣言することができます。
...
自動的な暗黙の引数は宣言の {tech}[ヘッダ] で使用されるパラメータのみを挿入するため、定義の右側にのみ現れる宇宙変数は `autoImplicit` が `true` の場合でも `universe` で宣言されていない限り引数として挿入されません。
...
`u` が `:=` の前にあるヘッダで言及されていないため、自動的な暗黙でもこの定義は却下されます：
