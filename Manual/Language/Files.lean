...
: ブロックコメント
  ...
  直後に `-` が続かず、字句の一部として出現しない `/-` は _ブロックコメント_ を開始します。 {index (subterm := "block")}[comment] `/--` はコメントではなくドキュメント文字列を開始します。
...
{deftech}[識別子] （identifier）は `'.'` 区切りで1つ以上の識別子要素から構成されます。 {index}[identifier]
...
識別子要素は二重ギュメ（`'«'` と `'»'`）で囲むこともできます。このような識別子要素には `'»'` 以外であれば `'«'` や改行文字でさえも含めたどのような文字でも含めることができます。
...
モジュールには環境が含まれます。これには環境からのデータ型と定数定義、環境拡張に格納されたデータの両方が含まれます。Lean によってモジュールが処理されると、コマンドは環境に内容を追加します。モジュールの環境は {deftech (key:="olean")}[`.olean` ファイル] にシリアライズすることができ、これには環境と環境によって必要とされるランタイムオブジェクトを含むコンパクト化されたヒープ領域の両方を含みます。これはインポートされたモジュールが、そのコマンドをすべて再実行することなくロードできることを意味します。
