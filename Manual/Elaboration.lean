...
: パース
  ...
  パーサは文字のシーケンスを {lean}`Syntax` 型の構文木に変換します。Lean のパーサは拡張可能であるため、 {lean}`Syntax` のデータ型は非常に一般的です。
...
{lean}`everyOther.eq_unfold` は完全に適用されていない `everyOther` がその展開と等しいことを示します：
